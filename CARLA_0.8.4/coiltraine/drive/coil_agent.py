...
class CoILAgent(object):
    ...
    def __init__(self, checkpoint, town_name, carla_version='0.84'):
        ...
        if g_conf.USE_MOCO:
            self.moco_model = Build_MoCo()
            self.moco_model.cuda()
    ...
    def run_step(self, measurements, sensor_data, directions, target, path):
        ...
        model_outputs = self._model.forward_branch(self._process_sensors(sensor_data, path), norm_speed,
                                                  directions_tensor)
    ...
    def _process_sensors(self, sensors, path):
        ...
        if g_conf.IMAGE_TRANSLATION:
          ...
          if g_conf.USE_MOCO:
              image_input = self.moco_model.moco.generate_representation((image_input.cuda()))
