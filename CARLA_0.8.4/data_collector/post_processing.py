...
def reshape_images(image_type, episode, data_point_number):
    ...
    if center is None:
        ...
        purge(episode, "measurement_" + data_point_number+".json")
        return
    
    if center.shape[0] == 600:
        center = center[IMAGE_CUT[0]:IMAGE_CUT[1], ...]
       
        center = cv2.resize(center, (200, 88), interpolation=cv2.INTER_CUBIC)
    ...
    for episode in episodes_list[args.start_episode:]:
        ...
        try:
            ...
            for measurement in measurements_list[:-3]:
                ...
                if 'forwardSpeed' in  measurement_data:
                    speed = measurement_data['forwardSpeed']
                else:
                    speed = 0
