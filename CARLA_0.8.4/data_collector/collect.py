...
def start_carla_simulator(gpu, town_name, docker):
    ...
    sp = subprocess.Popen(
            ['docker', 'run', '--rm', '-d', '-p', str(port) + '-' + str(port + 2) + 
                ':' + str(port) + '-' + str(port + 2), '--gpus=' + str(gpu), 
                '-e', 'SDL_VIDEODRIVER=offscreen', docker,
                '/bin/bash', 'CarlaUE4.sh', '/Game/Maps/' + town_name, '-windowed',
                '-benchmark', '-fps=.0001', '-world-port=' + str(port)], shell=False, 
            stdout=subprocess.PIPE)
...
def collect(client, args):
    ...
    while (os.path.exists(os.path.join(args.data_path, 'episode_' + str(episode_number).zfill(5)))):
            episode_number+=1
    ...
    try:
        ...
        while carla_game.is_running() and episode_number < maximun_episode:
            ...
            print("col", collision_checker.test_collision(measurements.player_measurements))
            print(carla_game.is_reset(measurements.player_measurements.transform.location))
            ...
            # episode number.
            print("Success", episode_success)
            print("Num", episode_number)

            if episode_ended:
                print("Done", episode_success)
                if episode_success:
                    episode_number += 1
                else:
...
def main():
    ...
    carla_process, port, out = start_carla_simulator(0, 'Town01',
                                                         'carlagear')

    while True:
        try:
            
            with make_carla_client(args.host, port) as client:
                collect(client, args)
                break
